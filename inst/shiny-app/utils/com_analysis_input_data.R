parsed_input_data_util <- function(input, results, i18n) {
  ...
  if (input$input_type == "single") {
    ...
    # Check if user uploaded a file OR load default
    if (is.null(input$csv_file)) {
      # Load default CSV
      default_path <- system.file("extdata", "gui_com_input-test-data-0.csv", package = "supeRcrit")
      if (file.exists(default_path)) {
        tryCatch({
          data <- read.csv(default_path, header = TRUE, stringsAsFactors = FALSE,
                          na.strings = c("", "NA", "N/A"), strip.white = TRUE)
          # Remove empty columns
          data <- data[, !sapply(data, function(x) all(is.na(x) | x == ""))]

          # Return with default column names (response -> yield)
          final_data <- data.frame(time = data$time, yield = data$response)
          results$is_multi_value <- nrow(final_data) > 1
          return(final_data)
        }, error = function(e) {
          showNotification(paste(i18n$t("Error loading default file:"), e$message), type = "error")
          results$is_multi_value <- FALSE
          return(NULL)
        })
      } else {
        showNotification(i18n$t("Default data file not found."), type = "error")
        results$is_multi_value <- FALSE
        return(NULL)
      }
    }

    # User uploaded file - continue with existing logic
