...
#' @param k1_0 Optional \code{numeric} vector of starting value(s) for \eqn{k_1}. When a vector of
#' length > 1 is provided, multi-start optimization is performed. Overrides the \code{k1} element of
#' \code{opt_est}. Defaults to \code{NULL} (uses \code{opt_est}).
#' @param k2_0 Optional \code{numeric} vector of starting value(s) for \eqn{k_2}. Same behavior as \code{k1_0}.
#' @param nfits Number of times to repeat the entire fitting process (default \code{1}).
#' When > 1, the best fit (lowest AARD) is used for the model, and standard deviations across fits
#' are reported in the output \code{$tws$fit_summary}.
#'
#'
...
ktsmod <- function(oec, oec_vars, pars, units = "default", opt_est = "default",
  ...
  # Build k1/k2 starting value vectors
  k1_vec <- if (!is.null(k1_0)) unique(as.numeric(k1_0)) else opt_est[["k1"]]
  k2_vec <- if (!is.null(k2_0)) unique(as.numeric(k2_0)) else opt_est[["k2"]]

  modres[["tws"]] <- tws_cmp(x = t, y = yield, c0 = c0, q = q, f = if(!est_f) f else NA,
                             k1_0 = k1_vec, k2_0 = k2_vec,
                             f0 = if(est_f) opt_est[["f"]] else NA, est_f = est_f, maxt = tmax, maxq = qmax,
                             optmet = optmet, nfits = nfits)
