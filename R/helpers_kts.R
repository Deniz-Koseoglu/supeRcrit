...
moderr <- function(act, pred, m = length(act)) {
  ...
  #Get error metrics - handle zero values in act for AARD calculation
  valid_idx <- act != 0
  n_valid <- sum(valid_idx)
  if(n_valid == 0) {
    aard <- NA
    warning("All actual values are zero; AARD cannot be calculated.")
  } else {
    aard <- 100 * sum(abs((pred[valid_idx] - act[valid_idx]) / act[valid_idx])) / n_valid
  }
...
#' @param k1_0,k2_0 Initial estimate(s) for first-order rate constants \eqn{k_1} and \eqn{k_2}.
#' Can be single values or \code{numeric} vectors for multi-start optimization (max 6 values recommended).
...
#' @param nfits Number of times to repeat the fitting process (default \code{1}). When > 1, the best fit
#' is selected, and standard deviations across fits are included in \code{$fit_summary}.
#'
...
tws_cmp <- function(x, y, c0, q = NA, f = NA, k1_0 = 0.5, k2_0 = 0.5, f0 = NA, est_f = TRUE, modpts = 100,
  ...
  # Multi-start: k1_0 and k2_0 can be vectors
  k1_starts <- unique(k1_0)
  k2_starts <- unique(k2_0)
  nfits <- max(1L, as.integer(nfits))

  # Storage for nfits repetitions
  all_fit_results <- list()

  for(fit_iter in seq_len(nfits)) {
  ...
  if(optmet == "nlrob") {
    ...
    for(k1s in k1_starts) {
      for(k2s in k2_starts) {
        optlims <- list(lower = c(k1 = k1s/200, k2 = k2s/200, f = if(est_f) f0/200 else NULL),
                        upper = c(k1 = k1s*200, k2 = k2s*200, f = if(est_f) 1 else NULL))

        #FITTING ROUND 1
        set.seed(42 + fit_iter - 1)
        resmat_r1 <- as.data.frame(matrix(nrow = 4, ncol = 2, dimnames = list(c(), c("k1","k2"))))
        if(est_f) resmat_r1[,"f"] <- rep(NA,4)
        for(i in seq_along(nlsm)) {
          ...
          if(!inherits(optres, "try-error")) resmat_r1[i,] <- coef(optres)
        }

        #Get mean and SD values for round 2
        k1_rd2 <- mean(resmat_r1[,"k1"], na.rm = TRUE)
        k2_rd2 <- mean(resmat_r1[,"k2"], na.rm = TRUE)
        f_rd2 <- if(est_f) mean(resmat_r1[,"f"], na.rm = TRUE) else NA
        k1_sd <- sd(resmat_r1[,"k1"], na.rm = TRUE)
        k2_sd <- sd(resmat_r1[,"k2"], na.rm = TRUE)
        f_sd <- if(est_f) sd(resmat_r1[,"f"], na.rm = TRUE) else NA

        if(!is.na(k1_rd2) && !is.na(k2_rd2)) {
          ...
          for(i in seq_along(nlsm)) {
            ...
            if(!inherits(optres, "try-error")) {
              ...
              aard_val <- moderr(pred = optpred[2:m], act = cc0[2:m], m = m-1)[["aard"]]
              all_aards <- c(all_aards, aard_val)
              all_coefs[[length(all_coefs) + 1]] <- cfs
            }
          }
        }
      }
    ...
    nlsm <- c("NLOPT_LN_COBYLA", "NLOPT_LN_BOBYQA", "NLOPT_LN_NEWUOA", "NLOPT_LN_PRAXIS")

    for(k1s in k1_starts) {
      for(k2s in k2_starts) {
        x0 <- c(k1s, k2s, if(est_f) f0 else NULL)
        ...
        for(i in seq_along(nlsm)) {
          ...
          if(!inherits(optres, "try-error")) {
            ...
            aard_val <- moderr(pred = optpred[2:m], act = cc0[2:m], m = m-1)[["aard"]]
            all_aards <- c(all_aards, aard_val)
            all_coefs[[length(all_coefs) + 1]] <- setNames(cfs, c("k1", "k2", "f"))
          }
        }
      }
    }
  ...
  #Get final estimated parameter values for this fit iteration
  if(length(all_aards) > 0) {
    if(aggreg == "aard") {
      best_idx <- which.min(all_aards)
      k1 <- all_coefs[[best_idx]][["k1"]]
      k2 <- all_coefs[[best_idx]][["k2"]]
      finf <- if(est_f) all_coefs[[best_idx]][["f"]] else f
    } else if(aggreg == "mean") {
      k1 <- mean(sapply(all_coefs, function(z) z[["k1"]]), na.rm = TRUE)
      k2 <- mean(sapply(all_coefs, function(z) z[["k2"]]), na.rm = TRUE)
      finf <- if(est_f) mean(sapply(all_coefs, function(z) z[["f"]]), na.rm = TRUE) else f
    }
    all_fit_results[[fit_iter]] <- c(k1 = k1, k2 = k2, f = finf,
                                      aard = min(all_aards, na.rm = TRUE))
  }

  } # end nfits loop

  # Aggregate across nfits repetitions
  if(length(all_fit_results) == 0) {
    warning("All optimization attempts failed. Check input data and parameters.")
    return(NULL)
  }

  fit_results_df <- as.data.frame(do.call(rbind, all_fit_results))
  # Use the best fit (lowest AARD) for the model
  best_fit <- which.min(fit_results_df$aard)
  k1 <- fit_results_df$k1[best_fit]
  k2 <- fit_results_df$k2[best_fit]
  finf <- fit_results_df$f[best_fit]

  # Compute SD across fits (meaningful when nfits > 1)
  fit_summary <- data.frame(
    Parameter = c("k1", "k2", if(est_f) "f"),
    Value = c(k1, k2, if(est_f) finf),
    Mean = c(mean(fit_results_df$k1), mean(fit_results_df$k2), if(est_f) mean(fit_results_df$f)),
    SD = c(sd(fit_results_df$k1), sd(fit_results_df$k2), if(est_f) sd(fit_results_df$f))
  )
  ...
  return(list(ordt = ordt, mdt = mod_dt, mod_pars = mod_pars, fit_pars = fit_pars, resid = resid,
              fit_summary = fit_summary, fit_results = fit_results_df))
}
