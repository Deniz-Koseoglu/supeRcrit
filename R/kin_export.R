...
kin_export <- function(modres, sumres = NA, expath = getwd(), which_mod = "all", plotpars = "default", plot_format = "png",
  ...
  # Only validate sumres if it's not NA and is a list
  if (is.list(sumres) && !all(is.na(sumres))) {
  ...
  if (any(which_mod %in% "all")) {
    which_mod <- defmods
  } else if (!all(which_mod %in% c(defmods))) {
  ...
  pltlst <- list()
  modnms <- c(
    "sim" = "Simple BIC model", "ct" = "BIC model based on characteristic times",
  ...
  if (!silent) cat("\nExporting plots...")
  plotsave <- lapply(seq_along(pltlst), function(x) {
    ggplot2::ggsave(paste0(expath_plots, "/", names(pltlst)[x], ".", plot_format), pltlst[[x]],
