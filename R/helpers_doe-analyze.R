...
doe_build <- function(input, resp_var, effs, orig_data = NA, uc_facs = NA) {
  ...
  if(is.character(uc_facs) && !any(is.na(uc_facs))) {
...
doe_optim <- function(model, mod_df, resp_var, effs, c_facs, uc_facs = NA) {
  ...
  if(!any(is.na(uc_facs)) & identical(opt_bounds, range(model[["model"]][,c_facs], na.rm = TRUE))) {
    ...
    decoded_result <- doe_decode(modsum[["tradopt"]][["coded"]], mod_df, c_facs, uc_facs, task = "decode")[["newvals"]]
    # Rename columns from coded (A, B, C) to uncoded (actual factor names)
    decode_map <- setNames(uc_facs, c_facs)
    colnames(decoded_result) <- ifelse(colnames(decoded_result) %in% names(decode_map),
                                        decode_map[colnames(decoded_result)],
                                        colnames(decoded_result))
    modsum[["tradopt"]][["decoded"]] <- decoded_result
  }
  return(modsum)
}
...
predict_doe <- function(input, newdata, coded = FALSE) {
  ...
  for(i in names(input[["models"]])) {
    ...
    if(!all(names(newdata) %in% c(ucfacs, cfacs))) stop("Unrecognized variables in prediction data! Check 'newdata'...")
    facinds <- which(names(newdata) %in% cfacs | names(newdata) %in% ucfacs)
    if(length(facinds) > 0) {
      if(!is.null(ucfacs)) ucfacs <- ucfacs[facinds]
      cfacs <- cfacs[facinds]
    }
    ...
    can_decode <- !is.null(ucfacs) && length(ucfacs) > 0 && !any(is.na(ucfacs)) && !identical(ucfacs, cfacs)
    ...
    if(can_decode) {
      ...
      cvals <- if(coded) newdata else stop("Uncoded predictions require all uncoded factor associations to be specified!")
      names(cvals) <- cfacs
      
      #Carry out prediction (coded only, no uncoded columns)
      preds[[i]] <- predict(curmod, newdata = cvals)
      df[[i]] <- setNames(cbind.data.frame(cvals, preds[[i]]), c(cfacs, respname))
    }
  }
  cat("\nDONE!")
