...
hsp_export <- function(input, addres = NA, plotpars = "default", plot_format = "png", expath = getwd(), silent = FALSE) {
  ...
  if(is.list(optres)) {
    ...
    if(length(modif_vec)>1) {
      ...
      colnames(miscib_best) <- paste0(colnames(miscib_best), " °C")
