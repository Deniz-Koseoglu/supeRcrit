...
doe_desir <- function(mods, dsrng, frng = "default", obj = rep("max", length(mods)), dtype = "coded",
  ...
  if (!is.list(dsrng) || length(dsrng) != length(mods) || !all(lengths(dsrng) %in% c(2, 3)) || !all(sapply(dsrng, is.numeric)) || !all(names(dsrng) %in% colnames(doe_df))) {
    stop("Argument 'dsrng' must be a list of numeric vectors of length 2 (or 3 for target objectives) containing minimum and maximum limits for responses!")
  ...
  if (length(trgchk) > 0) {
    wts <- lapply(seq_along(wts), function(x) {
      if (obj[x] == "trg") {
        if (length(wts[[x]]) == 1) rep(wts[[x]], 2) else wts[[x]]
      } else {
        wts[[x]]
      }
    })
  ...
  doe_df[, "DO"] <- sapply(seq(nrow(doe_df)), function(x) {
    vals <- doe_df[x, grep("di_", colnames(doe_df))] # d_overall(doe_df[,c(11:13)])
  ...
  for (i in seq_along(parlst)) {
    ...
    if (optmet == "optim") {
      ...
      optres <- suppressWarnings(try(optim(
        par = curpar, fn = desir_opt, method = "L-BFGS-B", lower = lbds,
        upper = ubds, eqs = eqs_opt, dsrng = dsrng, wts = wts, obj = obj,
        optype = "single", opt = "max", control = list(fnscale = -1, maxit = 1000)
      ), silent = TRUE))
      ...
      for (j in seq_along(nlpm)) {
        optres <- suppressWarnings(try(nloptr::nloptr(
          x0 = curpar,
          eval_f = desir_opt,
          lb = lbds,
          ub = ubds,
      ...
      if (length(slst) > 0 && length(ovec) > 0) {
        optres[[opnm]] <- slst[[which(ovec == max(ovec))[1]]]
      } else {
        next
      }
    }
