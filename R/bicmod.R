...
#' @param ksas0 Optional \code{numeric} vector of starting value(s) for \eqn{k_s \cdot a_s}.
#' When a vector of length > 1 is provided, multi-start optimization is performed: each value
#' generates its own search bounds, and the best result (lowest AARD) is selected.
#' Overrides the \code{ksas} element of \code{opt_est} when provided. Defaults to \code{NULL}
#' (uses the single value from \code{opt_est}).
#'
...
bicmod <- function(oec, oec_vars, pars, opt_est = "default", etoh_frac = 0, flowpar = rep(NA,2), ro_co2 = NA, tmax = NA, qmax = NA,
                   ...
                   units = "default", silent = FALSE, ksas0 = NULL) {
  ...
  # Build ksas starting values vector: ksas0 param overrides opt_est$ksas if provided
  ksas_vec <- if (!is.null(ksas0)) unique(as.numeric(ksas0)) else opt_est[["ksas"]]
  ...
  if(!est_cu) {
    if(n < 2) stop("Number of Observations (CER) must be at least 2 for polynomial fitting!")
  ...
  modres[["sim"]] <- bic_sm(yield, q, n, xu, qaver, Nm, porosity, ys, opt_est[["c3"]], Ng, est_cu = if(est_cu) TRUE else FALSE,
                            maxq = qmax, r0 = opt_est[["r"]], ksas0 = ksas_vec, qc0 = opt_est[["qc"]])
  ...
  if(any(c("cmp3", "cmp2") %in% modtype)) {
    modres[["cmp"]] <- bic_cmp(yield, q, n, a0, gam, porosity, qaver, Nm, ys, xu,
                               r0 = modres[["sim"]][["mod_pars"]][["r"]], ksas0 = unique(c(modres[["sim"]][["mod_pars"]][["ksas"]], ksas_vec)),
