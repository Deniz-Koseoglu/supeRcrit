...
#' @param ksas0 Initial estimate(s) of the product of the solid phase mass transfer coefficient (\eqn{k_s}; 1/s)
#' and the specific area between intact and broken cells (\eqn{a_s}; 1/m), in 1/m/s.
#' Can be a single value or a \code{numeric} vector for multi-start optimization (max 6 values recommended).
...
bic_sm <- function(yield, q, n, xu = NA, qaver, Nm = NA, porosity, ys = NA, c3 = NA, Ng = NA, est_cu = FALSE,
  ...
  #nlrob() approach - supports multi-start when ksas0 is a vector
  set.seed(42)
  ...
  for(ks in unique(ksas0)) {
    optlims <- list(lower = c(c1 = r0/20, c2 = ks/20, c3 = if(est_cu) c3/20 else NULL),
                    upper = c(c1 = r0*20, c2 = ks*20, c3 = if(est_cu) 1 else NULL))
    for(i in seq_along(nlsm)) {
      optres <- try(robustbase::nlrob(sd_formula,
                                      data = optdata,
                                      lower = optlims[["lower"]],
                                      upper = optlims[["upper"]], method = nlsm[i]), silent = TRUE)
      if(!inherits(optres, "try-error")) {
        cfs <- coef(optres)
        if(!est_cu) {
          e_opt <- xu * (1 - cfs[1] * exp(-cfs[2] * q[n:m]))
          aard_val <- moderr(pred = e_opt, act = yield[n:m])[["aard"]]
        } else {
          aard_val <- 0
        }
  ...
  # Check if all optimizations failed
  if(length(all_aards) == 0 && !est_cu) {
    warning("All optimization methods failed in the simplified model. Check input data and parameters.")
    return(NULL)
  }

  if(aggreg == "aard" && length(all_aards) > 0) {
    best_idx <- which.min(all_aards)
    c1 <- all_coefs[[best_idx]][["c1"]]
    c2 <- all_coefs[[best_idx]][["c2"]]
  } else {
    c1 <- mean(sapply(all_coefs, function(x) x[["c1"]]), na.rm = TRUE)
    c2 <- mean(sapply(all_coefs, function(x) x[["c2"]]), na.rm = TRUE)
  }

  if(est_cu) {
    best_idx <- which.min(all_aards)
    c3 <- all_coefs[[best_idx]][["c3"]]
    cu_est <- c3 / (1 + c3)
  ...
  qm <- tryCatch(
    uniroot(solv_sim, interval = c(qc0/20, qc0*20), extendInt = "yes", tol = 1e-10, maxiter = 1000, xu = xu, c1 = c1, c2 = c2, ys = ys)[["root"]],
    error = function(e) {
      # Fallback: minimize |solv_sim(x)|^2 via optimize
      opt_res <- optimize(function(x) solv_sim(x, xu = xu, c1 = c1, c2 = c2, ys = ys)^2,
                          interval = c(1e-8, qc0 * 200))
      opt_res$minimum
    }
  )
...
bic_cmp <- function(yield, q, n, a0, gam, porosity, qaver, Nm, ys, xu, r0, ksas0, kf0 = 0.001, qc0 = 400,
  ...
  # Support multi-start: ksas0 can be a vector
  ksas_vals <- unique(c(ksas0))
  cmp_optdata <- data.frame(y2 = y2, q2 = q2,
                             kf = rep(kf, length(q2)),
                             xu = rep(xu, length(q2)),
                             ys = rep(ys, length(q2)),
  ...
  for(ks in ksas_vals) {
    for(i in seq_along(nlsm)) {
      optres <- suppressWarnings(try(robustbase::nlrob(
        y2 ~ fer_cmp_eq(r, ksas, q2, kf, xu, ys, porosity, qaver, gam, Nm, a0),
        data = cmp_optdata,
        lower = c(r = r0/2, ksas = ks/5),
        upper = c(r = r0*2, ksas = ks*5),
        method = nlsm[i]), silent = TRUE))
      if(!inherits(optres, "try-error")) {
        cfs <- coef(optres)
        optpred <- fer_cmp_eq(cfs[["r"]], cfs[["ksas"]], q2, kf, xu, ys, porosity, qaver, gam, Nm, a0)
        aard_val <- moderr(pred = optpred, act = y2, m = length(y2))[["aard"]]
        all_aards <- c(all_aards, aard_val)
        all_coefs[[length(all_coefs) + 1]] <- cfs
      }
    }
  ...
  if(length(all_aards) == 0) {
    warning("All optimization methods failed in the complete model. Check input data and parameters.")
    return(NULL)
  }

  if(aggreg=="aard") {
    best_idx <- which.min(all_aards)
    r <- all_coefs[[best_idx]][["r"]]
    ksas <- all_coefs[[best_idx]][["ksas"]]
  } else if(aggreg == "mean") {
    r <- mean(sapply(all_coefs, function(x) x[["r"]]), na.rm = TRUE)
    ksas <- mean(sapply(all_coefs, function(x) x[["ksas"]]), na.rm = TRUE)
  }
